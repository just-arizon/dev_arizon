...
import { Menu, X, FileDown, ArrowRight } from "lucide-react";
import { Link } from "react-router-dom";
import Logo from "../assets/images/pro.jpg";
import resumePDF from "../assets/resume.pdf";

const navLinks = [
  { href: "https://github.com/just-arizon", label: "GitHub" },
              { href: "https://www.linkedin.com/in/arinze-calistus/", label: "LinkedIn" },
              { href: "https://x.com/just_Arizon", label: "Twitter" },
              { href: "https://www.youtube.com/@dev_arizon", label: "YouTube" }
];
    ...
    <>
      ...
      >
        <div className="container mx-auto flex justify-between items-center">
          {/* Logo */}
          <Link smooth to="/" className="flex items-center gap-3 group">
          ...
          <nav className="hidden md:flex items-center gap-8">
            {navLinks.map((social) => (
              <Link
                              ...
                              className="group inline-flex items-center gap-1.5 text-gray-500 hover:text-[#c084fc] text-sm transition-colors duration-200"
                            >
                              {social.label}
            ...
            <a
              ...
              className="font-clash flex items-center gap-2 text-sm font-medium px-4 py-2 rounded-lg border border-[#c084fc]/40 text-[#c084fc] hover:bg-[#c084fc]/10 hover:border-[#c084fc]/70 transition-all duration-200"
            >
          ...
          <div className="absolute top-0 right-0 h-full w-72 bg-[#0f0f0f] border-l border-white/10 flex flex-col pt-20 px-6 gap-2 shadow-2xl">
            {navLinks.map((link) => (
              <Link
                ...
                to={link.href}
                onClick={() => setMobileOpen(false)}
                className="text-lg text-gray-300 hover:text-white py-3 border-b border-white/5 transition-colors font-medium flex"
              >
                {link.label}
                <ArrowRight className="w-3.5 h-3.5 -rotate-45 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-transform duration-200" />
              </Link>
